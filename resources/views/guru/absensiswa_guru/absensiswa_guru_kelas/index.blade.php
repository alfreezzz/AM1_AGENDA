<x-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 p-4">
        ...
        <!-- Date Filter Form -->
        <form method="GET" action="{{ url('absensiswa_guru/kelas/' . $kelas->id) }}" class="flex items-center space-x-2">
            ...
            <a href="{{ url('absensiswa_guru/kelas/' . $kelas->id) }}" class="bg-gray-500 text-white py-2 px-4 rounded hover:bg-gray-600 transition duration-200">
                Reset
            </a>
        </form>
    </div>
