<x-layout>
    ...
    <div class="container mx-auto my-8">
        <div class="flex justify-center">
            <h1 class="text-center text-2xl font-bold mb-6 inline-block border-b-2 border-green-100 pb-1 text-green-700">Pilih Kelas untuk Melihat Rekapan Absensi Siswa</h1>
        </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-8 relative">
                @foreach ($kelas as $item)
                    @if (Str::startsWith(trim($item->kelas), 'X') && !Str::startsWith(trim($item->kelas), 'XI') && !Str::startsWith(trim($item->kelas), 'XII'))
                        <a href="{{ url('absen_siswa/kelas/' . $item->slug . '/rekapan') }}" class="block text-center py-4 bg-green-500 text-white rounded-lg shadow-md hover:bg-green-600 transition duration-300">
