    ...
    <div class="px-4 py-8 mx-auto max-w-7xl sm:px-4 lg:px-8">
        ...
        <div class="mb-12">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <form action="{{ url('user') }}" method="GET" id="filterForm" class="grid flex-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                ...
                                <tr class="transition duration-200 hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2">
                                        <div x-data="{ openMapel: false, openKelas: false }" class="space-y-3">
                                            <div>
                                                <button @click="openMapel = !openMapel" class="flex items-center w-full px-3 py-2 text-sm text-left transition duration-200 rounded-lg bg-gray-50 hover:bg-gray-100">
                                                    <span class="font-medium text-gray-700">Mapel</span>
                                                    <svg :class="{ 'rotate-180': openMapel }" class="w-5 h-5 ml-auto text-gray-400 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19.5 8.25-7.5 7.5-7.5-7.5"/>
                                                    </svg>
                                                </button>
                                                <div x-show="openMapel" class="pl-2 mt-2 space-y-2">
                                                    @forelse ($item['mapels'] as $mapel)
                                                        <span class="block bg-green-50 text-green-700 py-2.5 px-3 rounded-lg text-sm">
                                                            {{ $mapel['nama_mapel'] }}
                                                        </span>
                                                    @empty
                                                        <span class="text-sm text-gray-400">Tidak ada mapel</span>
                                                    @endforelse
                                                </div>
                                            </div>
                                            
                                            <div>
                                                <button @click="openKelas = !openKelas" class="flex items-center w-full px-3 py-2 text-sm text-left transition duration-200 rounded-lg bg-gray-50 hover:bg-gray-100">
                                                    <span class="font-medium text-gray-700">Kelas</span>
                                                    <svg :class="{ 'rotate-180': openKelas }" class="w-5 h-5 ml-auto text-gray-400 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19.5 8.25-7.5 7.5-7.5-7.5"/>
                                                    </svg>
                                                </button>
                                                <div x-show="openKelas" class="pl-2 mt-2 space-y-2">
                                                    ...
                                                    @empty
                                                        <span class="text-sm text-gray-400">Tidak ada kelas</span>
                                                    @endforelse
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <td class="px-4 py-2 text-center whitespace-nowrap">
                                        @if($item->kelas)
                                            <span class="inline-flex px-3 py-2 text-sm text-yellow-700 rounded-lg bg-yellow-50">
