    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->role === 'Guru') {
            ...
            foreach ($kelas as $kls) {
                ...
                // ✅ Tambahan: cek apakah absen siswa sudah diisi hari ini
                $absenSiswaHariIni = Absensiswa_Guru::where('kelas_id', $kls->id)
                    ->whereDate('tgl', now()->toDateString())
                    ->where('added_by', $user->id)
                    ->exists();

                $kls->sudah_diisi = $absenSiswaHariIni; // true kalau sudah diisi
            }
